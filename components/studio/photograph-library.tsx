...
export function PhotographLibrary({ photographs }: { photographs: Photograph[] }) {
  ...
  return <section aria-label="照片资料库" className="studio-library">
    ...
    <div className="studio-library-list" role="tabpanel">
      {visiblePhotographs.length === 0 ? <div className="studio-empty-state"><p className="studio-kicker">{activeTab === "draft" ? "01 / DRAFTS" : "02 / PUBLISHED"}</p><h2>{activeTab === "draft" ? "还没有待整理的照片。" : "还没有公开的摄影作品。"}</h2><p>上传功能将在下一步加入；每一张照片都会先以草稿身份进入你的档案。</p></div> : visiblePhotographs.map((photograph, index) => <article className="studio-photograph-row" key={photograph.id}><span aria-hidden="true" className="studio-row-number">{String(index + 1).padStart(2, "0")}</span><div><h2>{photograph.title ?? "未命名照片"}</h2><p>{photograph.location ?? "位置待补充"} · {formatDate(photograph.createdAt)}</p></div><span className={`studio-status studio-status-${photograph.status}`}>{labels[photograph.status]}</span></article>)}
    </div>
  </section>;
}
